...
const FabContext = React.createContext<Omit<FabProps, 'onClick'>>({ variant: 'primary' });
...
const Fab = ({
  ...
  return (
    <FabContext.Provider value={{ variant, size }}>
      <FabStl variant={variant} size={size} elevation={elevation} onClick={onClick}>
        {children}
      </FabStl>
    </FabContext.Provider>
  );
...
const FabIcon = (props: IconProps) => {
  ...
  return (
    <Icon
      {...props}
      color={IconVariant[variant]}
      size={IconSize[size ?? 'small']}
    />
  );
};
...
export const FabStl = styled.button<FabProps>(
  ({ theme, variant, size, elevation }) => ({
    height: 56,
    width: 56,
    border: 0,
    padding: 0,
    backgroundColor: theme.color[ContainerVariant[variant]],
    boxShadow: elevationFn(
